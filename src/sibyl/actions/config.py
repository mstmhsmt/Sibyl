...
class ActionConfig(Action):
    ...
    def run(self):
        if self.args.dump:
            ...
            if self.args.value.endswith("_keys") and hasattr(config, self.args.value[:-5]):
                val = list(getattr(config, self.args.value[:-5]).keys())
    ...
    def show(self):
        ...
        for name, tests in config.available_tests.items():
